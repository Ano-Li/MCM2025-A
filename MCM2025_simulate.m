...
% 单高斯和双高斯的比例 beta
beta_values = [0.35 0.65]; % 单高斯占比，双高斯占比是 1-beta
...
for idx = 1:length(beta_values)
    ...
    caxis(c_range); % 设置颜色范围
    colormap(custom_colormap);
    title(['Upward Ratio: ', num2str(alpha), ', Beta: ', num2str(beta)], 'FontSize', 13);
    xlabel('Step Length (m)');
    ylabel('Step Width (m)');
    zlabel('Step Height (m)');
    axis tight;
